...
class Pet:
    def __init__(self, nome, especie, idade, peso, nome_dono, vacinado, altura=0.0, raca="", temperamento="", castrado=False, hospedado=False):
        self.nome = nome
    ...
    def exibir_dados(self):
        ...
        print(f"Castrado: {'Sim' if self.castrado else 'Não'}")
    ...
    def criar_de_dicionario(dados):
        ...
        return Pet(
            ...
            dados["nome_dono"],
            dados["vacinado"],
            dados.get("altura", 0.0),
            dados.get("raca", ""),
            dados.get("temperamento", ""),
            dados.get("castrado", False),
...
def cadastrar_pet(lista_pets):
    ...
    castrado_resposta = input("O pet é castrado? (s/n): ").lower()
    castrado = castrado_resposta == "s"
    nome_dono = input("Nome do dono: ")

    resposta = input("O pet está vacinado? (s/n): ").lower()
    vacinado = resposta == "s"

    pet = Pet(nome, especie, idade, peso, nome_dono, vacinado, altura, raca, temperamento, castrado)
    lista_pets.append(pet)
