...
def carregar_pets(arquivo):
    '''Carrega a lista de pets de um arquivo binário pickle, se existir. Trata arquivo inexistente.'''
...
def escolher_pet(pets, acao):
    '''Retorna um pet escolhido pelo usuário ou None se inválido. Valida índices.'''
    if not pets:
    ...
    while True:
        ...
        if escolha.isdigit():
            ...
            if 0 <= indice < len(pets):
                ...
                print(f'Índice inválido. Digite um número entre 1 e {len(pets)}.')
                continue
        print('Escolha inválida. Digite um número válido ou ENTER para cancelar.')


def check_in(pets):
    '''Realiza check-in de um pet escolhido.'''
...
def check_out(pets):
    '''Realiza check-out de um pet escolhido.'''
...
def atualizar_peso_pet(pets):
    '''Atualiza o peso de um pet escolhido.'''
...
def main():
    '''Função principal que executa o menu interativo do hotel de pets.'''
